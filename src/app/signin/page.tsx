...
export default function AuthPage() {
  ...
  // Tracks whether to display 'signin' layout or 'signup' layout
  ...
  // Looks at the URL query parameter and dynamically updates the visual view state
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (view === 'signup') {
      ...
      try {
        // 1. Verify handle username uniqueness inside the backend data profile table
        ...
        // 2. Register user account credentials
        ...
        // 3. Landing routing shift straight into dashboard workspace
      ...
      // Direct Login Configuration Execution
      try {
    ...
    <div style={{ display: 'flex', minHeight: '100vh', width: '100vw', alignItems: 'center', justifyContent: 'center', backgroundColor: '#FAF8F5', fontFamily: 'sans-serif', color: '#111111', padding: '20px', boxSizing: 'border-box' }}>
      <div style={{ backgroundColor: '#ffffff', width: '100%', maxWidth: '440px', padding: '40px', borderRadius: '24px', border: '1px solid #E8E2D9', boxShadow: '0 4px 25px rgba(0,0,0,0.03)', boxSizing: 'border-box' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* USERNAME INPUT CONTAINER - Extends dynamically when state view is 'signup' */}
          {view === 'signup' && (
            <div>
          ...
          {/* PASSWORD CONFIRM CONTAINER - Extends dynamically when state view is 'signup' */}
          {view === 'signup' && (
            <div>
