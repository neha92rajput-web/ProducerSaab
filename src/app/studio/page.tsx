...
export default function StudioWorkspace() {
  ...
  // Workspace Modes: 'personal' shows your profile, 'community' shows the LinkedIn/Insta network layout
  ...
  const [allProducers, setAllProducers] = useState<any[]>([]); // Global profiles for community deck
  ...
  useEffect(() => {
    async function loadStudioData() {
      ...
      // Fetch YOUR real profile info dynamically
      const { data: record } = await database
        .from('profiles')
        .select('*')
        .eq('id', user.id)
        .maybeSingle();
      ...
      if (record) {
        ...
        // Fallback placeholder using your real email chunk instead of a hardcoded mock name
        const freshHandle = user.email?.split('@')[0] || 'producer';
        const defaultProfile = { display_name: freshHandle, username: freshHandle, headline: 'Music Producer | Mixer', pronouns: '', company: 'Independent Studio', location: 'Chandigarh, India' };
      ...
      // Fetch all community producers for the LinkedIn sub-dashboard layout
      const { data: globalProfiles } = await database.from('profiles').select('*').limit(20);
  ...
  const handleProfileSave = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      alert("Error saving properties to database layer.");
    }
  };

  if (loading) return <div className="min-h-screen bg-[#F3F2EF] flex items-center justify-center text-xs">Opening Studio Engine...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#F3F2EF] text-[#191919] pb-12 font-sans antialiased">
      ...
      {/* HEADER DOCK BAR NAVIGATION */}
      <header className="sticky top-0 z-50 bg-white border-b border-gray-200 px-6 py-3 shadow-sm">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button onClick={() => router.push('/')} className="text-gray-500 hover:text-black text-xs font-bold">← Global Feed</button>
            <span className="text-gray-300">|</span>
            <span className="text-xs font-black uppercase tracking-wider text-blue-700">Studio Room 💼</span>
          </div>
          
          {/* LINKEDIN LAYOUT SUB DASHBOARD NAVIGATION OPTION CONTROLS */}
          <div className="flex items-center gap-2">
            <button 
              onClick={() => setViewMode('personal')} 
              className={`text-xs font-bold px-4 py-1.5 rounded-full border transition ${viewMode === 'personal' ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
            >
              My Profile
            </button>
            <button 
              onClick={() => setViewMode('community')} 
              className={`text-xs font-bold px-4 py-1.5 rounded-full border transition ${viewMode === 'community' ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
            >
              Producer Community 👥
            </button>
          </div>
      ...
      <div className="max-w-3xl mx-auto mt-4 space-y-4 px-2 sm:px-0">
        ...
        {/* VIEW MODE A: PERSONAL DASHBOARD WORKSPACE */}
        {viewMode === 'personal' && (
          <>
            {/* WORKSPACE PROFILE IDENTITY CARD */}
            <div className="bg-white border border-gray-200 rounded-lg overflow-hidden relative shadow-sm">
              ...
              <div className="px-6 pb-6 relative">
                <div className="w-20 h-20 bg-black rounded-full border-4 border-white absolute -top-10 left-6 overflow-hidden flex items-center justify-center text-white font-bold text-2xl">
                  {profile.avatar_url ? <img src={profile.avatar_url} className="w-full h-full object-cover" alt="Avatar" /> : <span>{userInitial}</span>}
                </div>
                ...
                <div className="pt-12 space-y-1">
                  ...
                  <p className="text-xs text-gray-700 font-medium">{profile.headline || 'Music Producer | Mixer'}</p>
            ...
            {editingProfile && (
              <form onSubmit={handleProfileSave} className="bg-white border border-blue-200 rounded-lg p-5 space-y-3 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] text-gray-400 font-bold uppercase mb-1">Display Name</label>
                  ...
                  <div className="col-span-2 grid grid-cols-3 gap-2">
                    ...
                    <input type="text" className="border p-2 text-xs rounded bg-gray-50" placeholder="Studio Company" value={editForm.company || ''} onChange={(e) => setEditForm({ ...editForm, company: e.target.value })} />
                  ...
                  <input type="url" className="border p-2 text-xs rounded bg-gray-50 col-span-2" placeholder="Avatar photo direct URL link" value={editForm.avatar_url || ''} onChange={(e) => setEditForm({ ...editForm, avatar_url: e.target.value })} />
                  <input type="url" className="border p-2 text-xs rounded bg-gray-50 col-span-2" placeholder="Cover image direct URL link" value={editForm.cover_url || ''} onChange={(e) => setEditForm({ ...editForm, cover_url: e.target.value })} />
                </div>
            ...
            {/* ACTION CONTROLS ENGINE COMPONENT */}
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm space-y-3">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Share Hub:</span>
              ...
              {shareType === 'post' && (
                <div className="border-t pt-3 space-y-2">
                  <textarea value={postContent} onChange={(e) => setPostContent(e.target.value)} placeholder="What's on your mind? Broadcast loop, stem parameters or thoughts..." className="w-full text-xs p-3 bg-gray-50 border rounded-xl focus:outline-none min-h-[70px] resize-none" />
                  <div className="flex justify-end"><button onClick={handleCreatePost} disabled={publishingPost || !postContent.trim()} className="px-4 py-1.5 bg-blue-700 text-white font-bold text-xs rounded-full">Publish update</button></div>
                </div>
              )}
              ...
              {shareType === 'audio' && (
                <form onSubmit={handlePublishTrack} className="border-t pt-3 space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    ...
                    <select className="border text-xs p-2.5 rounded bg-white" value={trackGenre} onChange={(e) => setTrackGenre(e.target.value)}>
                    ...
                    <select className="border text-xs p-2.5 rounded bg-white" value={trackBpm} onChange={(e) => setTrackBpm(e.target.value)}>
                      <option value="90">90 BPM</option>
                      <option value="120">120 BPM</option>
                      <option value="140">140 BPM</option>
                    </select>
                    <select className="border text-xs p-2.5 rounded bg-white" value={trackKey} onChange={(e) => setTrackKey(e.target.value)}>
                      <option value="F# Minor">F# Minor</option>
                      <option value="C Major">C Major</option>
                      <option value="A Minor">A Minor</option>
                    </select>
                    <select className="border text-xs p-2.5 rounded bg-white" value={trackMood} onChange={(e) => setTrackMood(e.target.value)}>
            ...
            {/* PERSONAL MASTER AUDIO REPOSITORY INDEX */}
            <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm space-y-3">
              <h3 className="text-xs font-black uppercase tracking-widest text-gray-400">📊 Personal Audio Catalog</h3>
              <div className="space-y-2">
                {mySounds.length > 0 ? (
                  mySounds.map((track) => (
                    <div key={track.id} className="bg-gray-50 p-3 rounded-xl border flex justify-between items-center text-xs">
                      <div>
                  ...
                  <div className="text-xs text-gray-400 italic">No audio assets uploaded to your storage bucket line yet.</div>
                )}
              </div>
            </div>
          </>
        ...
        {/* VIEW MODE B: LINKEDIN/INSTAGRAM STYLE SUB DASHBOARD COMMUNITY NETWORK FEED */}
        {viewMode === 'community' && (
          <div className="space-y-4 animate-fadeIn">
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              <h3 className="text-xs font-black uppercase tracking-widest text-blue-700 mb-1">👥 Global Networks Dashboard</h3>
              <p className="text-[11px] text-gray-400 font-medium">Browse, follow, and connect with all verified audio architects registered on the platform ecosystem.</p>
            </div>

            {/* Scrolling grid layout of other user cards mimicking network parameters */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {allProducers.map((creator) => {
                const initials = String(creator.display_name || creator.username || 'P').charAt(0).toUpperCase();
                return (
                  <div key={creator.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm flex flex-col justify-between">
                    ...
                    <div className="p-4 pt-0 relative flex-1 flex flex-col justify-between">
                      <div className="w-12 h-12 bg-black border-2 border-white rounded-full absolute -top-6 left-4 overflow-hidden flex items-center justify-center text-white text-xs font-bold shadow-sm">
                        {creator.avatar_url ? <img src={creator.avatar_url} className="w-full h-full object-cover" /> : <span>{initials}</span>}
                      </div>
