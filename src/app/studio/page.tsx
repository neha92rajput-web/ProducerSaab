...
export default function CollaborationHub({ profileId }: CollabHubProps) {
  ...
  // Fetch incoming and outgoing collab requests cleanly
  const fetchRequests = async () => {
    if (!profileId) return;
    try {
      // 1. Get Requests sent TO you
  ...
  // Update request status (Accept / Decline) without page reloads
  const updateRequestStatus = async (requestId: string, nextStatus: 'accepted' | 'declined') => {
    try {
      ...
      fetchRequests(); // Refresh layout numbers
    } catch (err: any) {
