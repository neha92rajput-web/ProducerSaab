  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (authData.user) {
      await database.from('profiles').insert([
        { id: authData.user.id, username, account_type: accountType }
      ]);
      router.push('/studio');
    }
    setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center p-6">
      <form onSubmit={handleSignup} className="w-full max-w-sm space-y-6">
        ...
        <input placeholder="Username" onChange={(e) => setUsername(e.target.value)} className="w-full p-3 border border-[#E3DEC1] rounded-full text-sm" />
        <input type="email" placeholder="Email" onChange={(e) => setEmail(e.target.value)} className="w-full p-3 border border-[#E3DEC1] rounded-full text-sm" />
        <input type="password" placeholder="Password" onChange={(e) => setPassword(e.target.value)} className="w-full p-3 border border-[#E3DEC1] rounded-full text-sm" />

        <div className="space-y-3">
          <label className="text-[10px] font-black uppercase tracking-widest text-[#A4927A]">Choose your role</label>
